 ...
 * 插件访问日志表
 **/
class table_ddoa_log extends discuz_table
{
	public function __construct() {
		$this->_table = 'ddoa_log';
		$this->_pk = 'logid';
		parent::__construct();
	}
	...
	{/*{{{*/
		...
		$sday  = ddoa_validate::getNCParameter('sday','sday','string'); 
		$eday  = ddoa_validate::getNCParameter('eday','eday','string'); 
		$key   = ddoa_validate::getNCParameter('key','key','string'); 
        $sort  = ddoa_validate::getOPParameter('sort','sort','string',1024,'logtime');
        $dir   = ddoa_validate::getOPParameter('dir','dir','string',1024,'DESC');
        $start = ddoa_validate::getOPParameter('start','start','integer',1024,0);
        $limit = ddoa_validate::getOPParameter('limit','limit','integer',1024,20);
        $where = "date(logtime) BETWEEN date('$sday') AND date('$eday')";
		if ($key!="") $where.=" AND (log_content like '%$key%' OR b.username='$key')";
		$table_ddoa_log = DB::table($this->_table);
		$table_common_member = DB::table('common_member');
		$sql = <<<EOF
SELECT SQL_CALC_FOUND_ROWS a.*,b.username,b.email
FROM $table_ddoa_log as a LEFT JOIN $table_common_member as b ON a.uid=b.uid
...
FROM $table_ddoa_log WHERE $where GROUP BY vday ORDER BY logtime ASC
EOF;
