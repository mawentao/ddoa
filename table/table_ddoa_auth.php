 ...
 * 用户权限表
 **/
class table_ddoa_auth extends discuz_table
{
    public function __construct() {
		$this->_table = 'ddoa_auth';
		$this->_pk = 'uid';
		parent::__construct();
	}
    ...
    public function set()
    {
        $uid = ddoa_validate::getNCParameter('uid','uid','integer');
        $auth = ddoa_validate::getNCParameter('auth','auth','integer');
        $ctime = date("Y-m-d H:i:s");
        $sql = "INSERT INTO ".DB::table('ddoa_auth')." (uid,auth,ctime) VALUES ('$uid','$auth','$ctime') ".
	...
	public function query()
	{
        ...
        $auth  = ddoa_validate::getNCParameter('auth','auth','integer');
        $key   = ddoa_validate::getNCParameter('key','key','string',128);
        $sort  = ddoa_validate::getOPParameter('sort','sort','string',1024,'uid');
        $dir   = ddoa_validate::getOPParameter('dir','dir','string',1024,'ASC');
...
EOF;
        ...
        foreach ($return["root"] as &$row) {
            $row['groupname'] = C::m('#ddoa#ddoa_usergroup')->grouptitle($row['groupid']);
