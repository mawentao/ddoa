...
// 插件设置
$params = C::m('#ddoa#ddoa_nav_setting')->get();

// 保存设置
if (isset($_POST["reset"])) {
	if ($_POST["reset"]==1) {
		C::m('#ddoa#ddoa_nav_setting')->reset();
	} else {
        $menumap = array();
        ...
        // 排序
        ddoa_utils::array_sort_by($menumap,'displayorder','ASC');
        foreach ($menumap as $k => &$v) {
            if (!empty($v['subitems'])) {
                ddoa_utils::array_sort_by($v['subitems'],'displayorder','ASC');
            }   
        }   
        $params['navmenu'] = array_values($menumap);
        //die(json_encode($params));
        C::m('#ddoa#ddoa_nav_setting')->set($params);
	}
    $landurl = 'action=plugins&operation=config&do='.$pluginid.'&identifier=ddoa&pmod=z_nav';
...
ddoa_utils::loadtpl(dirname(__FILE__).'/template/views/z_nav.tpl', $params, $tplVars);
ddoa_env::getlog()->trace("show admin page [z_nav] success");
